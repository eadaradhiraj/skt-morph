...
// ---------------------------------------------------------------------------
// fn `vowel_initial_lang_stem`: purpose, inputs→outputs, edge cases.
...
// ---------------------------------------------------------------------------
// fn `bidadi_present_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `bidadi_lang_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `bidadi_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_bidadi`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_bidadi(antarganas: &str) -> bool { antarganas.contains("BidAdi") }
// ---------------------------------------------------------------------------
// fn `is_yajadi`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_yajadi(antarganas: &str) -> bool { antarganas.contains("yajAdi") }
// ---------------------------------------------------------------------------
// fn `is_gawadi`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_gawadi(antarganas: &str) -> bool { antarganas.contains("GawAdi") }

// ---------------------------------------------------------------------------
// fn `g1_rv_nv_present_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// const `G1_NV_ROOTS`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
const G1_NV_ROOTS: &[&str] = &["Dinv"];
// ---------------------------------------------------------------------------
// fn `g1_nv_present_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g1_nv_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `yam_cc_lang_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `yam_cc_future_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// const `G1_A_FINAL`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
const G1_A_FINAL: &[&str] = &["SrA","jYA"];
// ---------------------------------------------------------------------------
// const `BIDADI_THEMATIC`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
const BIDADI_THEMATIC: &[&str] = &["mid","med","meD","vap","vas","tF","guh"];

// ---------------------------------------------------------------------------
// fn `uses_aya_present`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_g1_a_final`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_g1_a_final(dhatu: &str) -> bool { G1_A_FINAL.contains(&dhatu) }
// ---------------------------------------------------------------------------
// fn `is_g1_aya_present`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_g1_aya_present(dhatu: &str) -> bool { G1_AYA_PRESENT.contains(&dhatu) }
// ---------------------------------------------------------------------------
// fn `is_g1_nv_root`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn is_g1_nv_root(dhatu: &str) -> bool { G1_NV_ROOTS.contains(&dhatu) }

// ---------------------------------------------------------------------------
// const `G9_N_INFIX`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g9_r_lang_root`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `ya_present_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `sad_present_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g6_present_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// const `G6_PLOT_LANG`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
const G6_PLOT_LANG: &[&str] = &["Brajj","Kid","kft","lip","lup","majj","muc","piS","pracC","sic","stfMh","SuB","tvac","vid"];

// ---------------------------------------------------------------------------
// fn `g6_plot_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g6_lang_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g6_lang_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// const `CAUSATIVE_GUNA_AY` — sūtra: 7.3.84/7.3.86 guṇa: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
const CAUSATIVE_GUNA_AY: &[&str] = &["yam","cap","cah","rah","bal","jYap"];

// ---------------------------------------------------------------------------
// const `_CAUSATIVE_LANG_BASE`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub const _CAUSATIVE_LANG_BASE: &[&str] = &["ci","jYA","kfp","Gf","kFt","cyu","BU","lI","gUd","gup","uDras","Card","raMh","mfj"];
// ---------------------------------------------------------------------------
// const `_CAUSATIVE_LANG_NO_AUG`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub const _CAUSATIVE_LANG_NO_AUG: &[&str] = &["uDras","Una","anDa","aMsa","aNka","aNga"];

// ---------------------------------------------------------------------------
// fn `causative_aya_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `causative_present_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
pub fn causative_present_stem(dhatu: &str) -> String { causative_aya_base(dhatu) }

// ---------------------------------------------------------------------------
// fn `lang_geminate_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `apply_vrddhi_to_stem` — sūtra: 7.2.115/116 vṛddhi: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `causative_lang_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `causative_lang_base_inner`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `causative_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g2_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g6_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g7_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `g9_vidhilin_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `thematic_aya_present_stem`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `thematic_present_base`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `thematic_join` — tin/sUP endings: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `apply_nasal_palatal`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `apply_natva_to_word`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
