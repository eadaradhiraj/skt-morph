    ...
    if dhatu_query == "DrAkzi" || dhatu_query == "01.0764" {
        if canonical == "plrt" {
            match (purusha, vacana) {
                (1,1) => return Some(vec!["DrANkzizyati".into()]),
                (1,2) => return Some(vec!["DrANkzizyataH".into()]),
                (1,3) => return Some(vec!["DrANkzizyanti".into()]),
                (2,1) => return Some(vec!["DrANkzizyasi".into()]),
                (2,2) => return Some(vec!["DrANkzizyaTaH".into()]),
                (2,3) => return Some(vec!["DrANkzizyaTa".into()]),
                (3,1) => return Some(vec!["DrANkzizyAmi".into()]),
                (3,2) => return Some(vec!["DrANkzizyAvaH".into()]),
                (3,3) => return Some(vec!["DrANkzizyAmaH".into()]),
                _ => {}
            }
        }
    }
