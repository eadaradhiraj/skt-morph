...
//! =============================================================================
//! src/engine/lit.rs: Pāṇini/Kaumudī implementation — extreme commenting pass (2026-09-01)
//! ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// struct `Angas`: purpose, inputs→outputs, edge cases.
...
// ---------------------------------------------------------------------------
// fn `prakriya_root`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_cac`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_a_plus_cons`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `ic_adi`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_guru_root`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `join_am`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `abhyasa`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `vrddhi_upadha` — sūtra: 7.2.115/116 vṛddhi: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `lopa_upadha`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `e_grade_cac`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `vacadi_angas`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `split_onset_vowel`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `is_cluster_cac`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `angas`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// fn `paradigm`: purpose, inputs→outputs, edge cases.
// Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `gam_lit_prathama_ekavacana_is_jagama`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `gam_weak_and_thal`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `pat_lit_e_grade`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn pat_lit_e_grade() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `khan_lit`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn khan_lit() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `abhyasa_kuho`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn abhyasa_kuho() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `gadh_nath_spardh_atmane`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `vac_lit_uvaca`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn vac_lit_uvaca() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `yaj_lit_iyaja`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn yaj_lit_iyaja() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `vap_vas_vad_svap`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn vap_vas_vad_svap() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `grah_lit`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn grah_lit() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `ni_kf_sru_lit`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn ni_kf_sru_lit() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `atmanepada_lit`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn atmanepada_lit() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `am_bru_jagf_nut`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn am_bru_jagf_nut() {
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `ig_upadha_and_tr_lit`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    ...
    #[test]
    // ---------------------------------------------------------------------------
    // fn `stu_cakas_thal`: purpose, inputs→outputs, edge cases.
    // Pāṇini step; see Kaumudī ordering. SLP1 I/O. No DB fallback.
    // ---------------------------------------------------------------------------
    fn stu_cakas_thal() {
