//! analyze — 8.2.66 is/us nap — safe real — reverse lookup (surface SLP1 → parses).
...
mod tests {
    ...
    #[test]
    fn analyze_all4_next37() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next38() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next39() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next40() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next41() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next42() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next43() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next44() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next45() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next46() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next47() {
        let h = crate::engine::analyze::analyze_word("gacchati");
        assert!(!h.is_empty());
    }

    #[test]
    fn analyze_all4_next48() {
        let h = crate::engine::analyze::analyze_word("rAmaH");
        assert!(!h.is_empty());
    }
