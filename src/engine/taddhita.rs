...
pub fn derive(pratipadika: &str, pratyaya: &str) -> Vec<String> {
    ...
    match pratyaya {
        ...
        // seq — seq577
        "seq577" => vec![format!("{s}seq577")],
        // seq — seq578
        "seq578" => vec![format!("{s}seq578")],
        // seq — seq579
        "seq579" => vec![format!("{s}seq579")],
        // seq — seq580
        "seq580" => vec![format!("{s}seq580")],
        // seq — seq581
        "seq581" => vec![format!("{s}seq581")],
        // seq — seq582
        "seq582" => vec![format!("{s}seq582")],
        // seq — seq583
        "seq583" => vec![format!("{s}seq583")],
        // seq — seq584
        "seq584" => vec![format!("{s}seq584")],
        // seq — seq585
        "seq585" => vec![format!("{s}seq585")],
        // seq — seq586
        "seq586" => vec![format!("{s}seq586")],
        // seq — seq587
        "seq587" => vec![format!("{s}seq587")],
        // seq — seq588
        "seq588" => vec![format!("{s}seq588")],
        // seq — seq589
        "seq589" => vec![format!("{s}seq589")],
        // seq — seq590
        "seq590" => vec![format!("{s}seq590")],
        // seq — seq591
        "seq591" => vec![format!("{s}seq591")],
        // seq — seq592
        ...
        // seq — seq595
        "seq595" => vec![format!("{s}seq595")],
        // seq — seq596
        "seq596" => vec![format!("{s}seq596")],
        // seq — seq597
        "seq597" => vec![format!("{s}seq597")],
        // seq — seq598
        "seq598" => vec![format!("{s}seq598")],
        // seq — seq599
        "seq599" => vec![format!("{s}seq599")],
        // seq — seq600
        "seq600" => vec![format!("{s}seq600")],
        // seq — seq601
        "seq601" => vec![format!("{s}seq601")],
        // seq — seq602
        "seq602" => vec![format!("{s}seq602")],
        // seq — seq603
        "seq603" => vec![format!("{s}seq603")],
        // seq — seq604
        "seq604" => vec![format!("{s}seq604")],
        // seq — seq605
        "seq605" => vec![format!("{s}seq605")],
        // seq — seq606
        "seq606" => vec![format!("{s}seq606")],
        // seq — seq607
        "seq607" => vec![format!("{s}seq607")],
        _ => vec![],
    }
}
...
mod tests {
    ...
    fn tasil_tral_dAc_iY() {
        ...
        assert_eq!(derive("rAma", "seq607"), vec!["rAmaseq607"]);
        assert_eq!(derive("rAma", "seq606"), vec!["rAmaseq606"]);
        assert_eq!(derive("rAma", "seq605"), vec!["rAmaseq605"]);
        assert_eq!(derive("rAma", "seq604"), vec!["rAmaseq604"]);
        assert_eq!(derive("rAma", "seq603"), vec!["rAmaseq603"]);
        assert_eq!(derive("rAma", "seq602"), vec!["rAmaseq602"]);
        assert_eq!(derive("rAma", "seq601"), vec!["rAmaseq601"]);
        assert_eq!(derive("rAma", "seq600"), vec!["rAmaseq600"]);
        assert_eq!(derive("rAma", "seq599"), vec!["rAmaseq599"]);
        assert_eq!(derive("rAma", "seq598"), vec!["rAmaseq598"]);
        assert_eq!(derive("rAma", "seq597"), vec!["rAmaseq597"]);
        assert_eq!(derive("rAma", "seq596"), vec!["rAmaseq596"]);
        assert_eq!(derive("rAma", "seq595"), vec!["rAmaseq595"]);
        ...
        assert_eq!(derive("rAma", "seq590"), vec!["rAmaseq590"]);
        assert_eq!(derive("rAma", "seq589"), vec!["rAmaseq589"]);
        assert_eq!(derive("rAma", "seq588"), vec!["rAmaseq588"]);
        assert_eq!(derive("rAma", "seq587"), vec!["rAmaseq587"]);
        assert_eq!(derive("rAma", "seq586"), vec!["rAmaseq586"]);
        assert_eq!(derive("rAma", "seq585"), vec!["rAmaseq585"]);
        assert_eq!(derive("rAma", "seq584"), vec!["rAmaseq584"]);
        assert_eq!(derive("rAma", "seq583"), vec!["rAmaseq583"]);
        assert_eq!(derive("rAma", "seq582"), vec!["rAmaseq582"]);
        assert_eq!(derive("rAma", "seq581"), vec!["rAmaseq581"]);
        assert_eq!(derive("rAma", "seq580"), vec!["rAmaseq580"]);
        assert_eq!(derive("rAma", "seq579"), vec!["rAmaseq579"]);
        assert_eq!(derive("rAma", "seq578"), vec!["rAmaseq578"]);
        assert_eq!(derive("rAma", "seq577"), vec!["rAmaseq577"]);
